#!/usr/bin/env bash
# Bring up 2x PURE TP4 GLM-5.2 replicas (MoE via tensor-parallel, no DP/EP) + nginx least_conn LB.
#
...
#
# Shared expert is FUSED into the MoE by default (FUSION_SHARED=1 -> fmoe key
# expert=257, topk=9). Set FUSION_SHARED=0 for the standalone shared-expert variant.
#
# Usage:
#   bash start_2tp4_pure_lb.sh          # start everything, wait until ready
#   bash start_2tp4_pure_lb.sh stop     # stop LB + both replicas
#
# Logs: /workspace/glm/srv_tp4pure_a.log , srv_tp4pure_b.log , /tmp/nginx_lb.err.log

set -u
GLM_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPLICA_SH="${GLM_DIR}/vllm_glm_fp4_prefill_tp4_pure.sh"
NGINX_CONF="${GLM_DIR}/nginx_lb.conf"
LOG_A="${GLM_DIR}/srv_tp4pure_a.log"
LOG_B="${GLM_DIR}/srv_tp4pure_b.log"
LB_PORT=8000
HEALTH_TIMEOUT=600   # seconds to wait per replica
FUSION_SHARED="${FUSION_SHARED:-1}"
...
if ps -eo comm | grep -q '^VLLM::'; then
    echo "ERROR: VLLM processes already running. Run 'bash start_2tp4_pure_lb.sh stop' first." >&2
    exit 1
fi
...
# ---- start replicas (setsid so they survive this shell exiting) ----
echo "[start] replica A: GPUs 0-3 -> :8001 (FUSION_SHARED=${FUSION_SHARED})"
setsid bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3 PORT=8001 FUSION_SHARED=${FUSION_SHARED} bash '${REPLICA_SH}' > '${LOG_A}' 2>&1" < /dev/null &
echo "[start] replica B: GPUs 4-7 -> :8002 (FUSION_SHARED=${FUSION_SHARED})"
setsid bash -c "CUDA_VISIBLE_DEVICES=4,5,6,7 PORT=8002 FUSION_SHARED=${FUSION_SHARED} bash '${REPLICA_SH}' > '${LOG_B}' 2>&1" < /dev/null &
...
echo " Stop everything with:     bash ${GLM_DIR}/start_2tp4_pure_lb.sh stop"
echo "==================================================================="
